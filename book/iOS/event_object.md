...
`JSONDictionary` is a method that returns a JSON formatted dictionary of all the properties on an event. This method will be used when we need JSON data to POST an event to the API. The `RFC3339DateFormatter` remains undefined, but we will address that in the next section.
