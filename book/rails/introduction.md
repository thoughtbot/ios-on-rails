...
not allow for multiple users per device or a single account across multiple devices, but
...
conveniently include all the gems we will use to test-drive our API,
...
“JSON”, we found it difficult to ask iOS developers to stay up to date with API changes
using source code on GitHub.
...
ensures that only known users can access the API. This also allows the API to turn off
access to a particular person or application if the usage violates the API terms
of service. [This blog post on API security]
(https://stormpath.com/blog/top-six-reasons-use-api-keys-and-how/)
...
zero security would mean anyone could create a "curl" request to an
...
value matching the value set by the Rails app can create new users via the
Humon API. We store the app secret in an environment variable so it is not
anywhere in version control.

We use ["dotenv"](https://github.com/bkeepers/dotenv) so our API can read
